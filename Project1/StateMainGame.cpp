    ...
    : stateManager(manager)
{
    ...
    // 初始化分数文本
    powerText.setFont(assetManager.getFont("simhei")); // 同上
    powerText.setCharacterSize(30);                      // 同上
    powerText.setFillColor(sf::Color::Black);            // 同上
    powerText.setPosition(680, 100);                     // 设置文本位置稍微低于时间文本
    ...
    healthWarning.setFillColor(sf::Color(255, 0, 0, 0)); // 初始透明度为0
...
void StateMainGame::update(float deltaTime) {
    ...
    if (!isEsc)
    {
        ...
        if (healthRatio < 0.4f) {
            // 生命值小于40%，开始显示警告
            int minAlpha = 15;  // 透明度下限设为30
            int maxAlpha = 100; // 透明度上限设为100
